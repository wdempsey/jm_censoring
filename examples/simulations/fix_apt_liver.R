...
apt_schedule = c(seq(0,1,1/3),seq(2,max(Table_1$survival),1))
...
Table_2$treat = factor(Table_2$treat, levels = c('null', 'control', 'prednisone') )
...
cov_lambda <- 1.67; Patient_cens.ds <- exp(-abs(outer(Table_2_rev$revival, Table_2_rev$revival, "-"))/cov_lambda) *Patient_cens # Patient Specific Exponential Covariance Matrix


model2 <- regress(Table_2_rev$obs ~ Table_2_rev$treat + Table_2_rev$survival + Table_2_rev$revival  + log(Table_2_rev$revival + delta), ~Patient_cens + Patient_cens.ds, kernel = 0)
